...
enum llm_kv {
    ...
    LLM_KV_FEED_FORWARD_LENGTH_LOWRANK_GATE,
    LLM_KV_FEED_FORWARD_LENGTH_LOWRANK_DOWN,
    LLM_KV_FEED_FORWARD_LENGTH_LOWRANK_UP,
    LLM_KV_ATTENTION_LENGTH_LOWRANK_QUERY,
    LLM_KV_ATTENTION_LENGTH_LOWRANK_KEY,
    LLM_KV_ATTENTION_LENGTH_LOWRANK_VALUE,
    LLM_KV_ATTENTION_LENGTH_LOWRANK_OUTPUT,
...
static const std::map<llm_kv, const char *> LLM_KV_NAMES = {
    ...
    { LLM_KV_FEED_FORWARD_LENGTH_LOWRANK_GATE,  "%s.feed_forward_length_lowrank_gate"  },
    { LLM_KV_FEED_FORWARD_LENGTH_LOWRANK_DOWN,  "%s.feed_forward_length_lowrank_down"  },
    { LLM_KV_FEED_FORWARD_LENGTH_LOWRANK_UP,    "%s.feed_forward_length_lowrank_up"  },
    { LLM_KV_ATTENTION_LENGTH_LOWRANK_QUERY,    "%s.attention_lowrank_query"           },
    { LLM_KV_ATTENTION_LENGTH_LOWRANK_KEY,      "%s.attention_lowrank_key"             },
    { LLM_KV_ATTENTION_LENGTH_LOWRANK_VALUE,    "%s.attention_lowrank_value"           },
    { LLM_KV_ATTENTION_LENGTH_LOWRANK_OUTPUT,   "%s.attention_lowrank_output"          },
...
enum llm_tensor {
    ...
    LLM_TENSOR_FFN_LR_GATE_A,
    LLM_TENSOR_FFN_LR_DOWN_A,
    LLM_TENSOR_FFN_LR_UP_A,
    LLM_TENSOR_FFN_LR_GATE_B,
    LLM_TENSOR_FFN_LR_DOWN_B,
    LLM_TENSOR_FFN_LR_UP_B,

    LLM_TENSOR_ATTN_LR_Q_A,
    LLM_TENSOR_ATTN_LR_Q_B,
    LLM_TENSOR_ATTN_LR_K_A,
    LLM_TENSOR_ATTN_LR_K_B,
    LLM_TENSOR_ATTN_LR_V_A,
    LLM_TENSOR_ATTN_LR_V_B,
    LLM_TENSOR_ATTN_LR_OUT_A,
    LLM_TENSOR_ATTN_LR_OUT_B,
...
static const std::map<llm_arch, std::map<llm_tensor, std::string>> LLM_TENSOR_NAMES = {
    {
        ...
        LLM_ARCH_LLAMA,
        {
            ...
            { LLM_TENSOR_FFN_LR_GATE_A,      "blk.%d.ffn_lr_gate_A" },
            { LLM_TENSOR_FFN_LR_DOWN_A,      "blk.%d.ffn_lr_down_A" },
            { LLM_TENSOR_FFN_LR_UP_A,        "blk.%d.ffn_lr_up_A" },
            { LLM_TENSOR_FFN_LR_GATE_B,      "blk.%d.ffn_lr_gate_B" },
            { LLM_TENSOR_FFN_LR_DOWN_B,      "blk.%d.ffn_lr_down_B" },
            { LLM_TENSOR_FFN_LR_UP_B,        "blk.%d.ffn_lr_up_B" },
    ...
    },
    {
        LLM_ARCH_QWEN2,
        {
            ...
            { LLM_TENSOR_ATTN_LR_Q_A,      "blk.%d.attn_lr_q_A" },
            { LLM_TENSOR_ATTN_LR_Q_B,      "blk.%d.attn_lr_q_B" },
            { LLM_TENSOR_ATTN_LR_K_A,        "blk.%d.attn_lr_k_A" },
            { LLM_TENSOR_ATTN_LR_K_B,      "blk.%d.attn_lr_k_B" },
            { LLM_TENSOR_ATTN_LR_V_A,      "blk.%d.attn_lr_v_A" },
            { LLM_TENSOR_ATTN_LR_V_B,        "blk.%d.attn_lr_v_B" },
            { LLM_TENSOR_ATTN_LR_OUT_A,        "blk.%d.attn_lr_output_A" },
            { LLM_TENSOR_ATTN_LR_OUT_B,        "blk.%d.attn_lr_output_B" },
        },
    },
    {
        LLM_ARCH_QWEN2MOE,
...
struct llama_hparams {
    ...
    std::array<uint32_t, LLAMA_MAX_LAYERS> n_inter_gate_arr;
    std::array<uint32_t, LLAMA_MAX_LAYERS> n_inter_down_arr;
    std::array<uint32_t, LLAMA_MAX_LAYERS> n_inter_up_arr;
    std::array<uint32_t, LLAMA_MAX_LAYERS> n_inter_q_arr;
    std::array<uint32_t, LLAMA_MAX_LAYERS> n_inter_k_arr;
    std::array<uint32_t, LLAMA_MAX_LAYERS> n_inter_v_arr;
    std::array<uint32_t, LLAMA_MAX_LAYERS> n_inter_out_arr;
    ...
    bool operator!=(const llama_hparams & other) const {
        ...
        if (this->n_inter_gate_arr      != other.n_inter_gate_arr)      return true;
        if (this->n_inter_down_arr      != other.n_inter_down_arr)      return true;
        if (this->n_inter_up_arr        != other.n_inter_up_arr)        return true;
        if (this->n_inter_q_arr         != other.n_inter_q_arr)         return true;
        if (this->n_inter_k_arr         != other.n_inter_k_arr)         return true;
        if (this->n_inter_v_arr         != other.n_inter_v_arr)         return true;
        if (this->n_inter_out_arr       != other.n_inter_out_arr)        return true;
    ...
    uint32_t n_inter_gate(uint32_t il = 0) const {
        if (il < n_layer) {
            return n_inter_gate_arr[il];
        }
    ...
    uint32_t n_inter_down(uint32_t il = 0) const {
        if (il < n_layer) {
            return n_inter_down_arr[il];
        }
    ...
    uint32_t n_inter_up(uint32_t il = 0) const {
        if (il < n_layer) {
            return n_inter_up_arr[il];
        }
    ...
    uint32_t n_inter_q(uint32_t il = 0) const {
        if (il < n_layer) {
            return n_inter_q_arr[il];
        }
    ...
    uint32_t n_inter_k(uint32_t il = 0) const {
        if (il < n_layer) {
            return n_inter_k_arr[il];
        }
    ...
    uint32_t n_inter_v(uint32_t il = 0) const {
        if (il < n_layer) {
            return n_inter_v_arr[il];
        }
    ...
    uint32_t n_inter_out(uint32_t il = 0) const {
        if (il < n_layer) {
            return n_inter_out_arr[il];
        }
...
struct llama_layer {
    ...
    struct ggml_tensor * ffn_gate_A; // w1
    struct ggml_tensor * ffn_down_A; // w2
    struct ggml_tensor * ffn_up_A;   // w3
    struct ggml_tensor * ffn_gate_B; // w1
    struct ggml_tensor * ffn_down_B; // w2
    struct ggml_tensor * ffn_up_B;   // w3
...
struct llama_model_loader {
    ...
    const struct ggml_tensor * check_tensor_dims(const std::string & name, const std::vector<int64_t> & ne, bool required) const {
        ...
        //std::cout << format("%s: tensor '%s' has shape; expected %s, got %s",
                            //__func__, name.c_str(),
                            //llama_format_tensor_shape(ne).c_str(),
                            //llama_format_tensor_shape(cur).c_str()) << std::endl;
...
static void llm_load_hparams(
    ...
    std::fill(hparams.n_inter_gate_arr.begin(),     hparams.n_inter_gate_arr.end(),      0);
    std::fill(hparams.n_inter_down_arr.begin(),     hparams.n_inter_down_arr.end(),      0);
    std::fill(hparams.n_inter_up_arr.begin(),       hparams.n_inter_up_arr.end(),        0);
    std::fill(hparams.n_inter_q_arr.begin(),        hparams.n_inter_q_arr.end(),         0);
    std::fill(hparams.n_inter_k_arr.begin(),        hparams.n_inter_k_arr.end(),         0);
    std::fill(hparams.n_inter_v_arr.begin(),        hparams.n_inter_v_arr.end(),         0);
    std::fill(hparams.n_inter_out_arr.begin(),      hparams.n_inter_out_arr.end(),       0);


    ml.get_key_or_arr(LLM_KV_FEED_FORWARD_LENGTH,               hparams.n_ff_arr,           hparams.n_layer);
    ml.get_key_or_arr(LLM_KV_FEED_FORWARD_LENGTH_LOWRANK_GATE,  hparams.n_inter_gate_arr,   hparams.n_layer);
    ml.get_key_or_arr(LLM_KV_FEED_FORWARD_LENGTH_LOWRANK_DOWN,  hparams.n_inter_down_arr,   hparams.n_layer);
    ml.get_key_or_arr(LLM_KV_FEED_FORWARD_LENGTH_LOWRANK_UP,    hparams.n_inter_up_arr,     hparams.n_layer);
    ml.get_key_or_arr(LLM_KV_ATTENTION_LENGTH_LOWRANK_QUERY,    hparams.n_inter_q_arr,      hparams.n_layer);
    ml.get_key_or_arr(LLM_KV_ATTENTION_LENGTH_LOWRANK_KEY,      hparams.n_inter_k_arr,      hparams.n_layer);
    ml.get_key_or_arr(LLM_KV_ATTENTION_LENGTH_LOWRANK_VALUE,    hparams.n_inter_v_arr,      hparams.n_layer);
    ml.get_key_or_arr(LLM_KV_ATTENTION_LENGTH_LOWRANK_OUTPUT,   hparams.n_inter_out_arr,    hparams.n_layer);
...
static bool llm_load_tensors(
    ...
    // create tensors for the weights
    {
        ...
        //const int64_t n_lr_start    = hparams.n_lr_start;
        //const int64_t n_lr_end      = hparams.n_lr_end;
        const int64_t n_ff              = hparams.n_ff();
        const int64_t n_inter_gate      = hparams.n_inter_gate();
        const int64_t n_inter_up        = hparams.n_inter_up();
        const int64_t n_inter_down      = hparams.n_inter_down();
        const int64_t n_inter_q         = hparams.n_inter_q();
        const int64_t n_inter_k         = hparams.n_inter_k();
        const int64_t n_inter_v         = hparams.n_inter_v();
        const int64_t n_inter_out       = hparams.n_inter_out();
            ...
            case LLM_ARCH_MINICPM:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        if (n_expert == 0) {
                            ...
                            //if (i > n_lr_end | i < n_lr_start ) {
                            //    layer.ffn_gate = ml.create_tensor(ctx_split, tn(LLM_TENSOR_FFN_GATE, "weight", i), {n_embd,   n_ff(i)});
                            //    layer.ffn_down = ml.create_tensor(ctx_split, tn(LLM_TENSOR_FFN_DOWN, "weight", i), {  n_ff(i), n_embd});
                            //    layer.ffn_up   = ml.create_tensor(ctx_split, tn(LLM_TENSOR_FFN_UP,   "weight", i), {n_embd,   n_ff(i)});
                            //} else {
                            //layer.ffn_gate_A = ml.create_tensor(ctx_split, tn(LLM_TENSOR_FFN_LR_GATE_A, "weight", i), {n_embd,   n_ff_2});
                            //layer.ffn_down_A = ml.create_tensor(ctx_split, tn(LLM_TENSOR_FFN_LR_DOWN_A, "weight", i), {  n_ff, n_ff_2});
                            //layer.ffn_up_A   = ml.create_tensor(ctx_split, tn(LLM_TENSOR_FFN_LR_UP_A,   "weight", i), {n_embd,   n_ff_2});
                            
                            //layer.ffn_gate_B = ml.create_tensor(ctx_split, tn(LLM_TENSOR_FFN_LR_GATE_B, "weight", i), {n_ff_2,   n_ff});
                            //layer.ffn_down_B = ml.create_tensor(ctx_split, tn(LLM_TENSOR_FFN_LR_DOWN_B, "weight", i), {  n_ff_2, n_embd});
                            //layer.ffn_up_B   = ml.create_tensor(ctx_split, tn(LLM_TENSOR_FFN_LR_UP_B,   "weight", i), {n_ff_2,   n_ff});
            ...
            case LLM_ARCH_QWEN2:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        const int64_t n_inter_gate    =   hparams.n_inter_gate(i);
                        const int64_t n_inter_up      =   hparams.n_inter_up(i);
                        const int64_t n_inter_down    =   hparams.n_inter_down(i);
                        const int64_t n_inter_q       =   hparams.n_inter_q(i);
                        const int64_t n_inter_k       =   hparams.n_inter_k(i);
                        const int64_t n_inter_v       =   hparams.n_inter_v(i);
                        const int64_t n_inter_out     =   hparams.n_inter_out(i);
                        ...
                        if (n_inter_q == 0) {
                            ...
                            layer.wqa = ml.create_tensor(ctx_split, tn(LLM_TENSOR_ATTN_LR_Q_A,   "weight", i), {n_embd, n_inter_q},     llama_model_loader::TENSOR_NOT_REQUIRED);
                            layer.wqb = ml.create_tensor(ctx_split, tn(LLM_TENSOR_ATTN_LR_Q_B,   "weight", i), {n_inter_q, n_embd},     llama_model_loader::TENSOR_NOT_REQUIRED);
                        }
                        
                        if (n_inter_k == 0) {
                            layer.wk = ml.create_tensor(ctx_split, tn(LLM_TENSOR_ATTN_K,   "weight", i), {n_embd, n_embd_gqa},     llama_model_loader::TENSOR_NOT_REQUIRED);
                        } else {
                            layer.wka = ml.create_tensor(ctx_split, tn(LLM_TENSOR_ATTN_LR_K_A,   "weight", i), {n_embd, n_inter_k},     llama_model_loader::TENSOR_NOT_REQUIRED);
                            layer.wkb = ml.create_tensor(ctx_split, tn(LLM_TENSOR_ATTN_LR_K_B,   "weight", i), {n_inter_k, n_embd_gqa},     llama_model_loader::TENSOR_NOT_REQUIRED);
                        }

                        if (n_inter_v == 0) {
                            layer.wv = ml.create_tensor(ctx_split, tn(LLM_TENSOR_ATTN_V,   "weight", i), {n_embd, n_embd_gqa},     llama_model_loader::TENSOR_NOT_REQUIRED);
                        } else {
                            layer.wva = ml.create_tensor(ctx_split, tn(LLM_TENSOR_ATTN_LR_V_A,   "weight", i), {n_embd, n_inter_v},     llama_model_loader::TENSOR_NOT_REQUIRED);
                            layer.wvb = ml.create_tensor(ctx_split, tn(LLM_TENSOR_ATTN_LR_V_B,   "weight", i), {n_inter_v, n_embd_gqa},     llama_model_loader::TENSOR_NOT_REQUIRED);
                        }

                        if (n_inter_out == 0) {
                            layer.wo = ml.create_tensor(ctx_split, tn(LLM_TENSOR_ATTN_OUT, "weight", i), {n_embd, n_embd},     llama_model_loader::TENSOR_NOT_REQUIRED);
                        } else {
                            layer.woa = ml.create_tensor(ctx_split, tn(LLM_TENSOR_ATTN_LR_OUT_A, "weight", i), {n_embd, n_inter_out},     llama_model_loader::TENSOR_NOT_REQUIRED);
                            layer.wob = ml.create_tensor(ctx_split, tn(LLM_TENSOR_ATTN_LR_OUT_B, "weight", i), {n_inter_out, n_embd},     llama_model_loader::TENSOR_NOT_REQUIRED);
                        }
                        ...
                        if (n_inter_gate == 0) {
                            layer.ffn_gate = ml.create_tensor(ctx_split, tn(LLM_TENSOR_FFN_GATE, "weight", i), {n_embd,   n_ff},     llama_model_loader::TENSOR_NOT_REQUIRED);
                        } else {
                            layer.ffn_gate_A = ml.create_tensor(ctx_split, tn(LLM_TENSOR_FFN_LR_GATE_A, "weight", i), {n_embd,   n_inter_gate},     llama_model_loader::TENSOR_NOT_REQUIRED);
                            layer.ffn_gate_B = ml.create_tensor(ctx_split, tn(LLM_TENSOR_FFN_LR_GATE_B, "weight", i), {n_inter_gate,   n_ff},     llama_model_loader::TENSOR_NOT_REQUIRED);
                        }

                        if (n_inter_down == 0) {
                            layer.ffn_down = ml.create_tensor(ctx_split, tn(LLM_TENSOR_FFN_DOWN, "weight", i), {  n_ff, n_embd},     llama_model_loader::TENSOR_NOT_REQUIRED);
                        } else {
                            layer.ffn_down_A = ml.create_tensor(ctx_split, tn(LLM_TENSOR_FFN_LR_DOWN_A, "weight", i), {  n_ff, n_inter_down},     llama_model_loader::TENSOR_NOT_REQUIRED);
                            layer.ffn_down_B = ml.create_tensor(ctx_split, tn(LLM_TENSOR_FFN_LR_DOWN_B, "weight", i), {  n_inter_down, n_embd},     llama_model_loader::TENSOR_NOT_REQUIRED);
                        }

                        if (n_inter_up == 0) {
                            layer.ffn_up   = ml.create_tensor(ctx_split, tn(LLM_TENSOR_FFN_UP,   "weight", i), {n_embd,   n_ff},     llama_model_loader::TENSOR_NOT_REQUIRED);
                        } else {
                            layer.ffn_up_A   = ml.create_tensor(ctx_split, tn(LLM_TENSOR_FFN_LR_UP_A,   "weight", i), {n_embd,   n_inter_up},     llama_model_loader::TENSOR_NOT_REQUIRED);
                            layer.ffn_up_B   = ml.create_tensor(ctx_split, tn(LLM_TENSOR_FFN_LR_UP_B,   "weight", i), {n_inter_up,   n_ff},     llama_model_loader::TENSOR_NOT_REQUIRED);
                        }
...
static struct ggml_tensor * llm_build_ffn_lowrank(
        struct ggml_context * ctx,
       struct llama_context & lctx,
         struct ggml_tensor * cur,
         struct ggml_tensor * up,
    ...
    if (up) {
        tmp = llm_build_lora_mm(lctx, ctx, up, cur);        
        cb(tmp, "ffn_up", il);

    } else if (up_A) {
        tmp = llm_build_lora_mm(lctx, ctx, up_A, cur);
        cb(tmp, "ffn_up_A", il);
        tmp = llm_build_lora_mm(lctx, ctx, up_B, tmp);
        cb(tmp, "ffn_up_B", il);
    } else {
        tmp = cur;
    }
    ...
    if (gate) {
        ...
        switch (type_gate) {
            case LLM_FFN_SEQ:
                {
                    tmp = llm_build_lora_mm(lctx, ctx, gate_A, tmp);    
                    cb(tmp, "ffn_gate_A", il);

                    cur = llm_build_lora_mm(lctx, ctx, gate_B, tmp);
                    cb(cur, "ffn_gate_B", il);
                } break;
            case LLM_FFN_PAR:
                {
                    cur = llm_build_lora_mm(lctx, ctx, gate_A, cur);
                    cb(cur, "ffn_gate_A", il);

                    cur = llm_build_lora_mm(lctx, ctx, gate_B, cur);
                    cb(cur, "ffn_gate_B", il);
                } break;
        }
    ...
    if (down) {
        ...
        cur = llm_build_lora_mm(lctx, ctx, down_A, cur);                    
        cb(cur, "ffn_down_A", il);

        cur = llm_build_lora_mm(lctx, ctx, down_B, cur);                    
...
struct llm_build_context {
    ...
    struct ggml_cgraph * build_llama() {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            if (model.layers[il].ffn_gate_inp == nullptr) {
                ...
                cur = llm_build_ffn_lowrank(ctx0, lctx, cur,
                    model.layers[il].ffn_up, model.layers[il].ffn_up_A, model.layers[il].ffn_up_B, NULL, NULL,
                    model.layers[il].ffn_gate, model.layers[il].ffn_gate_A, model.layers[il].ffn_gate_B, NULL, NULL,
                    model.layers[il].ffn_down, model.layers[il].ffn_down_A, model.layers[il].ffn_down_B, NULL, NULL,
                    NULL,
    ...
    struct ggml_cgraph * build_qwen2() {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            const int64_t n_inter_gate      = hparams.n_inter_gate(il);
            const int64_t n_inter_up        = hparams.n_inter_up(il);
            const int64_t n_inter_down      = hparams.n_inter_down(il);
            const int64_t n_inter_q         = hparams.n_inter_q(il);
            const int64_t n_inter_k         = hparams.n_inter_k(il);
            const int64_t n_inter_v         = hparams.n_inter_v(il);
            const int64_t n_inter_out       = hparams.n_inter_out(il);
            ...
            // self-attention
            {
                ...
                if (n_inter_q == 0) {
                    ...
                    Qcur = llm_build_lora_mm(lctx, ctx0, model.layers[il].wqa, cur);
                    cb(Qcur, "Qcur", il);
                    Qcur = llm_build_lora_mm(lctx, ctx0, model.layers[il].wqb, Qcur);
                    cb(Qcur, "Qcur", il);
                    Qcur = ggml_add(ctx0, Qcur, model.layers[il].bq);
                    cb(Qcur, "Qcur", il);
                }
                ...
                if (n_inter_k == 0) {
                    ...
                    Kcur = llm_build_lora_mm(lctx, ctx0, model.layers[il].wka, cur);
                    cb(Kcur, "Kcur", il);
                    Kcur = llm_build_lora_mm(lctx, ctx0, model.layers[il].wkb, Kcur);
                    cb(Kcur, "Kcur", il);
                    Kcur = ggml_add(ctx0, Qcur, model.layers[il].bk);
                    cb(Kcur, "Kcur", il);
                }
                ...
                if (n_inter_v == 0) {
                    ...
                    Vcur = llm_build_lora_mm(lctx, ctx0, model.layers[il].wva, cur);
                    cb(Vcur, "Vcur", il);
                    Vcur = llm_build_lora_mm(lctx, ctx0, model.layers[il].wvb, Vcur);
                    cb(Vcur, "Vcur", il);
                    Vcur = ggml_add(ctx0, Vcur, model.layers[il].bq);
                    cb(Vcur, "Vcur", il);
                }
